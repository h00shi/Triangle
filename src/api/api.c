...
void
triangle_context_destroy (context *ctx)
{
...
void
triangle_context_get_behavior (context *ctx, behavior *out)
{
  if (check_context (ctx) < 0)
    {
      return;
...
int
triangle_check_mesh (context *ctx)
{
  if (check_context (ctx) < 0)
    {
      return TRI_NULL;
...
int
triangle_check_delaunay (context *ctx)
{
  if (check_context (ctx) < 0)
    {
      return TRI_NULL;
...
int
triangle_mesh_load (context *ctx, triangleio *in)
{
  mesh *m;
  behavior *b;
  ...
  m->hullsize = reconstruct (m, b, in->trianglelist, in->triangleattributelist,
...
int
triangle_write_nodes (context *ctx, FILE *file)
{
  if (check_context (ctx) < 0)
    {
      return TRI_NULL;
...
int
triangle_write_elements (context *ctx, FILE *file)
{
  if (check_context (ctx) < 0)
    {
      return TRI_NULL;
...
int
triangle_write_poly (context *ctx, FILE *file, REAL *holelist, int holes,
                     REAL *regionlist, int regions)
{
  ...
  return file_writepoly (ctx->m, ctx->b, file, holelist, holes, regionlist,
                         regions);
}

int
triangle_write_edges (context *ctx, FILE *file)
{
  if (check_context (ctx) < 0)
    {
      return TRI_NULL;
...
int
triangle_write_neighbors (context *ctx, FILE *file)
{
  if (check_context (ctx) < 0)
    {
      return TRI_NULL;
...
int
triangle_write_eps (context *ctx, FILE *file)
{
  if (check_context (ctx) < 0)
    {
      return TRI_NULL;
...
int
triangle_read_nodes (const char *filename, triangleio *io, int *firstnode)
{
  int status;
...
int
triangle_read_poly (const char *filename, triangleio *io, int *firstnode)
{
  int status;
  FILE *file = fopen (filename, "r");
...
int
triangle_read_elements (const char *filename, triangleio *io)
{
  int status;
  FILE *file = fopen (filename, "r");
...
int
triangle_read_area (const char *filename, triangleio *io)
{
  int status;
  FILE *file = fopen (filename, "r");
