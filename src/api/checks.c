...
int
check_triangleio (triangleio *io, int firstnumber)
{
  int i;
  int a, b, c;
  REAL x1, y1, x2, y2;
  ...
  for (i = 0; i < insegments; i++)
    {
      ...
      if ((a < firstnumber) || (a >= firstnumber + invertices))
        ...
        {
          // TODO: Warning: Invalid second endpoint of segment (firstnumber +
          // i).
          return TRI_FAILURE;
        }
      else if (a == b)
        {
          // TODO: Warning: Endpoints of segment (firstnumber + i) are
          // coincident.
          return TRI_FAILURE;
        }
      else
...
int
restore_pointmarkers (context *ctx, int *pointmarkers)
{
  mesh *m;
  behavior *b;
  vertex vertexloop;
