...
int
file_writenodes (mesh *m, behavior *b, FILE *nodefile)
{
  vertex vertexloop;
  long outvertices;
  int vertexnumber;
...
int
file_writeelements (mesh *m, behavior *b, FILE *elefile)
{
  ...
  while (triangleloop.tri != (triangle *) NULL)
    {
      ...
      if (b->order == 1)
      ...
      else
        {
          ...
          fprintf (elefile, "%4ld    %4d  %4d  %4d  %4d  %4d  %4d",
                   elementnumber, vertexmark (p1), vertexmark (p2),
                   vertexmark (p3), vertexmark (mid1), vertexmark (mid2),
                   vertexmark (mid3));
        }
...
int
file_writepoly (mesh *m, behavior *b, FILE *polyfile, REAL *holelist, int holes,
...
int
file_writeedges (mesh *m, behavior *b, FILE *edgefile)
{
...
int
file_writeneighbors (mesh *m, behavior *b, FILE *neighborfile)
{
  struct otri triangleloop, trisym;
  long elementnumber;
...
int
file_write_eps (mesh *m, behavior *b, FILE *file)
{
  rect ps;
  rect clip;
  rect ms;
...
char *
readline (char *string, FILE *infile)
{
  char *result;
...
int
file_readnodes_internal (FILE *file, triangleio *io, int poly, int *firstnode)
{
  ...
  if (mesh_dim != 2)
    {
      return -1; // TODO: error: Triangle only works with two-dimensional
                 // meshes.
    }
  if (nextras == 0)
    {
  ...
  if (nextras)
    {
      io->pointattributelist
        = (REAL *) trimalloc (nextras * invertices * sizeof (REAL));
    }
  ...
  for (i = 0; i < invertices; i++)
    {
      ...
      if (*stringptr == '\0')
        {
          return -1; // TODO: error: Vertex (b->firstnumber + i) has no x
                     // coordinate.
        }
      ...
      if (*stringptr == '\0')
        {
          return -1; // TODO: error: Vertex (b->firstnumber + i) has no y
                     // coordinate.
        }
      ...
      for (j = 0; j < nextras; j++)
        {
          ...
          if (*stringptr == '\0')
            ...
            {
              io->pointattributelist[nextras * i + j]
...
int
file_readsegments_internal (FILE *file, triangleio *io)
{
  char inputline[INPUTLINESIZE];
  char *stringptr;
  ...
  for (i = 0; i < insegments; i++)
    {
      ...
      if (*stringptr == '\0')
        {
          return -1; // TODO: error: Segment (firstnumber + i) is missing its
                     // second endpoint.
        }
      else
        {
...
int
file_readholes_internal (FILE *file, triangleio *io)
{
  char inputline[INPUTLINESIZE];
  char *stringptr;
  int index;
      ...
      for (i = 0; i < 2 * holes; i += 2)
        {
          ...
          if (*stringptr == '\0')
            {
              return -1; // TODO: error: Hole (b->firstnumber + (i >> 1)) has no
                         // x coordinate.
            }
          else
            {
      ...
      for (i = 0; i < regions; i++)
        {
          ...
          if (*stringptr == '\0')
            {
              return -1; // TODO: error: Region (b->firstnumber + i) has no x
                         // coordinate.
            }
          else
            {
          ...
          if (*stringptr == '\0')
            {
              return -1; // TODO: error: Region (b->firstnumber + i) has no y
                         // coordinate.
            }
          else
            {
          ...
          if (*stringptr == '\0')
            {
              return -1; // TODO: error: Region (b->firstnumber + i) has no
                         // region attribute or area constraint.
            }
          else
            {
              io->regionlist[index++] = (REAL) strtod (stringptr, &stringptr);
...
int
file_readnodes (FILE *nodefile, triangleio *io, int *firstnode)
{
...
int
file_readpoly (FILE *polyfile, triangleio *io, int *firstnode)
{
  int s;
...
int
file_readelements (FILE *file, triangleio *io)
{
  char inputline[INPUTLINESIZE];
  char *stringptr;
  int inelements;
  ...
  if (eextras > 0)
    {
      ...
      io->triangleattributelist
        = (REAL *) trimalloc (eextras * inelements * sizeof (REAL));
    }
      ...
      for (j = 0; j < 3; j++)
        {
          ...
          if (*stringptr == '\0')
            {
              return -1; // TODO: error: Triangle (elementnumber) is missing
                         // vertex (j + 1).
            }
          else
            {
              io->trianglelist[3 * i + j]
                = (int) strtol (stringptr, &stringptr, 0);
            }
        }
      ...
      for (j = 0; j < eextras; j++)
        {
          ...
          if (*stringptr == '\0')
            ...
            {
              io->triangleattributelist[eextras * i + j]
                = (REAL) strtod (stringptr, &stringptr);
            }
        }
    }
...
int
file_readelementsarea (FILE *file, triangleio *io)
{
  char inputline[INPUTLINESIZE];
  char *stringptr;
  int areaelements;
