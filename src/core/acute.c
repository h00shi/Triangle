...
//=======================================//
// DATE: 06/15/2009
// GENERATES PREMIUM QUALITY TRIANGULATIONS; LARGE MINIMUM ANGLE VALUE OR LARGE
// MINIMUM ANGLE VALUE WHILE HAVING SMALL MAXIMUM ANGLE VALUE.
...
void
acutepool_init (int n, acutepool **mp)
{
...
void
acutepool_resize (int n, acutepool *p)
{
  if (p->size < n)
    {
      p->size = n;
...
void
findNewSPLocation (mesh *m, behavior *b, vertex torg, vertex tdest,
                   vertex tapex, vertex circumcenter, REAL *xi, REAL *eta,
                   int offcenter, struct otri badotri)
{
  // Based on using -U switch, call the corresponding function
  if (b->maxangle == 0.00000)
    {
      findNewSPLocationWithoutMaxAngle (m, b, torg, tdest, tapex, circumcenter,
                                        xi, eta, 1, badotri);
    }
  else
    {
      findNewSPLocationWithMaxAngle (m, b, torg, tdest, tapex, circumcenter, xi,
                                     eta, 1, badotri);
    }
...
/*********************************************************************************/
void
findNewSPLocationWithoutMaxAngle (mesh *m, behavior *b, vertex torg,
                                  vertex tdest, vertex tapex,
                                  vertex circumcenter, REAL *xi, REAL *eta,
                                  int offcenter, struct otri badotri)
{
  ...
  REAL xShortestEdge, yShortestEdge, xMiddleEdge, yMiddleEdge, xLongestEdge,
    yLongestEdge;

  // keeps the square of edge lengths
  REAL shortestEdgeDist, middleEdgeDist, longestEdgeDist;

  // keeps the vertices according to the angle incident to that vertex in a
  // triangle
  vertex smallestAngleCorner = NULL, middleAngleCorner = NULL,
         largestAngleCorner = NULL;

  // keeps the type of orientation if the triangle
  int orientation = 0;
  // keeps the coordinates of circumcenter of itself and neighbor triangle
  // circumcenter
  ...
  REAL xPetalCtr_1, yPetalCtr_1, xPetalCtr_2, yPetalCtr_2, xPetalCtr, yPetalCtr,
    xMidOfShortestEdge, yMidOfShortestEdge;
  ...
  REAL vector_x, vector_y, xMidOfLongestEdge, yMidOfLongestEdge, inter_x,
    inter_y, p[5], voronoiOrInter[4];
  int isCorrect = -1;
  ...
  REAL lengthConst = 1; // used at comparing circumcenter's distance to proposed
                        // point's distance
  REAL justAcute
    = 1; // used for making the program working for one direction only
  // for smoothing
  int relocated = 0; // used to differentiate between calling the deletevertex
                     // and just proposing a steiner point
  REAL newloc[2];    // new location suggested by smoothing
  REAL origin_x, origin_y; // for keeping torg safe
  struct otri
    delotri; // keeping the original orientation for relocation process
  // keeps the first and second direction suggested points
  REAL dxFirstSuggestion, dyFirstSuggestion, dxSecondSuggestion,
    dySecondSuggestion;
  ...
  ////
  //	longest: dodist		//	longest: dodist		//	longest:
  //dadist    //
  // 132: shortest: aodist 	// 231: shortest: dadist 	// 321: shortest:
  // dodist   //
  //	middle: dodist 		//	middle: dodist 		//	middle: dadist
  ////
  //	longest: dadist		//	longest: aodist		//	longest:
  //aodist    //
  /////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if (almostGood == 1)
    {
      ...
      cosMaxAngle = (middleEdgeDist + shortestEdgeDist - longestEdgeDist)
                    / (2 * sqrt (middleEdgeDist) * sqrt (shortestEdgeDist));
      if (cosMaxAngle < 0.0)
        {
          // obtuse
          isObtuse = 1;
      ...
      /// if relocation is possible, delete that vertex and insert a vertex at
      /// the new location ///
      if (relocated > 0)
        {
          dx = newloc[0] - torg[0];
      ...
      else
        {
          ...
          petalRadius
            = sqrt (shortestEdgeDist) / (2 * sin (b->minangle * PI / 180.0));
          /// compute two possible centers of the petal ///
          // finding the center
          // first find the middle point of smallest edge
          xMidOfShortestEdge
            = (middleAngleCorner[0] + largestAngleCorner[0]) / 2.0;
          yMidOfShortestEdge
            = (middleAngleCorner[1] + largestAngleCorner[1]) / 2.0;
          // two possible centers
          xPetalCtr_1
            = xMidOfShortestEdge
              + sqrt (petalRadius * petalRadius - (shortestEdgeDist / 4))
                  * (middleAngleCorner[1] - largestAngleCorner[1])
                  / sqrt (shortestEdgeDist);
          yPetalCtr_1
            = yMidOfShortestEdge
              + sqrt (petalRadius * petalRadius - (shortestEdgeDist / 4))
                  * (largestAngleCorner[0] - middleAngleCorner[0])
                  / sqrt (shortestEdgeDist);
          ...
          dxcenter1 = (xPetalCtr_1 - smallestAngleCorner[0])
                      * (xPetalCtr_1 - smallestAngleCorner[0]);
          dycenter1 = (yPetalCtr_1 - smallestAngleCorner[1])
                      * (yPetalCtr_1 - smallestAngleCorner[1]);
          dxcenter2 = (xPetalCtr_2 - smallestAngleCorner[0])
                      * (xPetalCtr_2 - smallestAngleCorner[0]);
          dycenter2 = (yPetalCtr_2 - smallestAngleCorner[1])
                      * (yPetalCtr_2 - smallestAngleCorner[1]);
          ...
          neighborNotFound
            = getNeighborsVertex (m, badotri, middleAngleCorner[0],
                                  middleAngleCorner[1], smallestAngleCorner[0],
                                  smallestAngleCorner[1], thirdPoint,
                                  &neighborotri);
          /// find the circumcenter of the neighbor triangle ///
          dxFirstSuggestion = dx; // if we cannot find any appropriate
                                  // suggestion, we use circumcenter
          ...
          if (neighborNotFound == 0)
            {
              ...
              findcircumcenter (m, b, neighborvertex_1, neighborvertex_2,
                                neighborvertex_3, neighborCircumcenter, &xi_tmp,
                                &eta_tmp, 0);
              /// compute petal and Voronoi edge intersection ///
              // in order to avoid degenerate cases, we need to do a vector
              // based calculation for line
              vector_x
                = (middleAngleCorner[1] - smallestAngleCorner[1]); //(-y, x)
              vector_y = smallestAngleCorner[0] - middleAngleCorner[0];
              vector_x = myCircumcenter[0] + vector_x;
              vector_y = myCircumcenter[1] + vector_y;

              // by intersecting bisectors you will end up with the one you want
              // to walk on
              // then this line and circle should be intersected
              circleLineIntersection (myCircumcenter[0], myCircumcenter[1],
                                      vector_x, vector_y, xPetalCtr, yPetalCtr,
                                      petalRadius, p);
              /// choose the correct intersection point ///
              // calculate middle point of the longest edge(bisector)
              xMidOfLongestEdge
                = (middleAngleCorner[0] + smallestAngleCorner[0]) / 2.0;
              yMidOfLongestEdge
                = (middleAngleCorner[1] + smallestAngleCorner[1]) / 2.0;
              // we need to find correct intersection point, since line
              // intersects circle twice
              isCorrect
                = chooseCorrectPoint (xMidOfLongestEdge, yMidOfLongestEdge,
                                      p[3], p[4], myCircumcenter[0],
                                      myCircumcenter[1], isObtuse);
              ...
              /// ///
              // check if the voronoi vertex is between the intersection and
              // circumcenter
              pointBetweenPoints (inter_x, inter_y, myCircumcenter[0],
                                  myCircumcenter[1], neighborCircumcenter[0],
                                  neighborCircumcenter[1], voronoiOrInter);
              ...
              if (p[0] > 0.0)
                { // there is at least one intersection point
                  ...
                  // if it returns 1.0 this means we have a voronoi vertex
                  // within feasible region
                  if (fabs (voronoiOrInter[0] - 1.0) <= compConst)
                    {
                      if (testTriangleAngle (
                            b, &middleAngleCorner[0], &middleAngleCorner[1],
                            &largestAngleCorner[0], &largestAngleCorner[1],
                            &neighborCircumcenter[0], &neighborCircumcenter[1])
                          != 0)
                        {
                    ...
                    { // there is no voronoi vertex between intersection point
                      // and circumcenter
                      if (testTriangleAngle (b, &largestAngleCorner[0],
                                             &largestAngleCorner[1],
                                             &middleAngleCorner[0],
                                             &middleAngleCorner[1], &inter_x,
                                             &inter_y)
                          != 0)
                        {
                          // if it is inside feasible region, then insert v2
                          // apply perturbation
                          // find the distance between circumcenter and
                          // intersection point
                          d = sqrt ((inter_x - myCircumcenter[0])
                                      * (inter_x - myCircumcenter[0])
                                    + (inter_y - myCircumcenter[1])
                                        * (inter_y - myCircumcenter[1]));
                          // then find the vector going from intersection point
                          // to circumcenter
                          ...
                          inter_x = inter_x
                                    + ax * pertConst * sqrt (shortestEdgeDist);
                          inter_y = inter_y
                                    + ay * pertConst * sqrt (shortestEdgeDist);
                          if (testTriangleAngle (b, &middleAngleCorner[0],
                                                 &middleAngleCorner[1],
                                                 &largestAngleCorner[0],
                                                 &largestAngleCorner[1],
                                                 &inter_x, &inter_y)
                              != 0)
                            {
                  ...
                  /// if it is an acute triangle, check if it is a good enough
                  /// location ///
                  // for acute triangle case, we need to check if it is ok to
                  // use either of them
                  if ((smallestAngleCorner[0] - myCircumcenter[0])
                          * (smallestAngleCorner[0] - myCircumcenter[0])
                        + (smallestAngleCorner[1] - myCircumcenter[1])
                            * (smallestAngleCorner[1] - myCircumcenter[1])
                      > lengthConst * ((smallestAngleCorner[0]
                                        - (dxFirstSuggestion + torg[0]))
                                         * (smallestAngleCorner[0]
                                            - (dxFirstSuggestion + torg[0]))
                                       + (smallestAngleCorner[1]
                                          - (dyFirstSuggestion + torg[1]))
                                           * (smallestAngleCorner[1]
                                              - (dyFirstSuggestion + torg[1]))))
                    {
                      // use circumcenter
            ...
            } // if it is on the boundary, meaning no neighbor triangle in this
              // direction, try other direction

          /// DO THE SAME THING FOR THE OTHER DIRECTION ///
          /// find the third point of the neighbor triangle  ///
          neighborNotFound
            = getNeighborsVertex (m, badotri, largestAngleCorner[0],
                                  largestAngleCorner[1], smallestAngleCorner[0],
                                  smallestAngleCorner[1], thirdPoint,
                                  &neighborotri);
          /// find the circumcenter of the neighbor triangle ///
          dxSecondSuggestion = dx; // if we cannot find any appropriate
                                   // suggestion, we use circumcenter
          ...
          if (neighborNotFound == 0)
            {
              ...
              // based calculation for line
              vector_x
                = (largestAngleCorner[1] - smallestAngleCorner[1]); //(-y, x)
              ...
              xMidOfMiddleEdge
                = (largestAngleCorner[0] + smallestAngleCorner[0]) / 2.0;
              yMidOfMiddleEdge
                = (largestAngleCorner[1] + smallestAngleCorner[1]) / 2.0;
              // we need to find correct intersection point, since line
              // intersects circle twice
              // this direction is always ACUTE
              isCorrect
                = chooseCorrectPoint (xMidOfMiddleEdge, yMidOfMiddleEdge, p[3],
                                      p[4], myCircumcenter[0],
                                      myCircumcenter[1], 0 /*(isObtuse+1)%2*/);
              // make sure which point is the correct one to be considered
              if (isCorrect == 1)
                {
                  inter_x = p[3];
              ...
              if (p[0] > 0.0)
                { // there is at least one intersection point
                  ...
                  if ((smallestAngleCorner[0] - myCircumcenter[0])
                      ...
                      > lengthConst
                          * ((smallestAngleCorner[0]
                              - (dxSecondSuggestion + torg[0]))
                               * (smallestAngleCorner[0]
                                  - (dxSecondSuggestion + torg[0]))
                             + (smallestAngleCorner[1]
                                - (dySecondSuggestion + torg[1]))
                                 * (smallestAngleCorner[1]
                                    - (dySecondSuggestion + torg[1]))))
                    {
                      // use circumcenter
              ...
              // direction, the other direction might be ok
          if (isObtuse == 1)
            {
              // obtuse: do nothing
...
/*********************************************************************************/
void
findNewSPLocationWithMaxAngle (mesh *m, behavior *b, vertex torg, vertex tdest,
                               vertex tapex, vertex circumcenter, REAL *xi,
                               REAL *eta, int offcenter, struct otri badotri)
{
  ...
  REAL minangle; // in order to make sure that the circumcircle of the bad
                 // triangle is greater than petal
  ...
  REAL line_p[3]; // used for getting the return values of functions related to
                  // line intersection
  REAL line_result[4];
  // intersection of slab and the petal
  REAL petal_slab_inter_x_first, petal_slab_inter_y_first,
    petal_slab_inter_x_second, petal_slab_inter_y_second, x_1, y_1, x_2, y_2;
  ...
  if (almostGood == 1)
    {
      ...
      if (relocated > 0)
      ...
      else
        {
          ...
          // first find the visible region, PETAL
          // find the center of the circle and radius
          // choose minimum angle as the maximum of quality angle and the
          // minimum angle of the bad triangle
          minangle
            = acos ((middleEdgeDist + longestEdgeDist - shortestEdgeDist)
                    / (2 * sqrt (middleEdgeDist) * sqrt (longestEdgeDist)))
              * 180.0 / PI;
          ...
          petalRadius
            = sqrt (shortestEdgeDist) / (2 * sin (minangle * PI / 180.0));
          /// compute two possible centers of the petal ///
          // finding the center
          // first find the middle point of smallest edge
          xMidOfShortestEdge
          ...
          dxFirstSuggestion = dx; // if we cannot find any appropriate
                                  // suggestion, we use circumcenter
          dyFirstSuggestion = dy;
          /// before checking the neighbor, find the petal and slab
          /// intersections ///
          ...
          dist = sqrt ((xPetalCtr - xMidOfShortestEdge)
                         * (xPetalCtr - xMidOfShortestEdge)
                       + (yPetalCtr - yMidOfShortestEdge)
                           * (yPetalCtr - yMidOfShortestEdge));
          ...
          x_1 = petal_bisector_x * cos (alpha) + petal_bisector_y * sin (alpha)
                + xPetalCtr - xPetalCtr * cos (alpha) - yPetalCtr * sin (alpha);
          y_1 = -petal_bisector_x * sin (alpha) + petal_bisector_y * cos (alpha)
                + yPetalCtr + xPetalCtr * sin (alpha) - yPetalCtr * cos (alpha);
          // rotate the vector ccw around the petal center
          x_2 = petal_bisector_x * cos (alpha) - petal_bisector_y * sin (alpha)
                + xPetalCtr - xPetalCtr * cos (alpha) + yPetalCtr * sin (alpha);
          y_2 = petal_bisector_x * sin (alpha) + petal_bisector_y * cos (alpha)
                + yPetalCtr - xPetalCtr * sin (alpha) - yPetalCtr * cos (alpha);
          // we need to find correct intersection point, since there are two
          // possibilities
          // weather it is obtuse/acute the one closer to the minimum angle
          // corner is the first direction
          isCorrect = chooseCorrectPoint (x_2, y_2, middleAngleCorner[0],
                                          middleAngleCorner[1], x_1, y_1, 1);
          // make sure which point is the correct one to be considered
          if (isCorrect == 1)
            {
          ...
          if (neighborNotFound_first == 0)
            {
              ...
              //----------------------hale new first direction: for slab
              //calculation---------------//
              ...
              // rotate the vector around middleAngleCorner in cw by maxangle
              // degrees
              linepnt2_x = petal_slab_inter_x_first;
              linepnt2_y = petal_slab_inter_y_first;
              // now calculate the intersection of two lines
              lineLineIntersection (myCircumcenter[0], myCircumcenter[1],
                                    vector_x, vector_y, linepnt1_x, linepnt1_y,
                                    linepnt2_x, linepnt2_y, line_p);
              ...
              if (line_p[0] > 0.0)
              ...
              else
                {
                  ...
                  printf ("(%.8f,%.8f) (%.8f,%.8f) (%.8f,%.8f) (%.8f,%.8f)\n",
                          myCircumcenter[0], myCircumcenter[1], vector_x,
                          vector_y, linepnt1_x, linepnt1_y, linepnt2_x,
                          linepnt2_y);
#endif
                }
                  ...
                  if (fabs (voronoiOrInter[0] - 1.0) <= compConst)
                    {
                      //-----------------hale new continues
                      //1------------------//
                      // now check if the line intersection is between cc and
                      // voronoi
                      pointBetweenPoints (voronoiOrInter[2], voronoiOrInter[3],
                                          myCircumcenter[0], myCircumcenter[1],
                                          line_inter_x, line_inter_y,
                                          line_result);
                      if (fabs (line_result[0] - 1.0) <= compConst
                          && line_p[0] > 0.0)
                        {
                          // check if we can go further by picking the slab line
                          // and petal intersection
                          // calculate the distance to the smallest angle corner
                          // check if we create a bad triangle or not
                          if (((smallestAngleCorner[0]
                                - petal_slab_inter_x_first)
                                   * (smallestAngleCorner[0]
                                      - petal_slab_inter_x_first)
                                 + (smallestAngleCorner[1]
                               ...
                               > lengthConst
                                   * ((smallestAngleCorner[0] - line_inter_x)
                                      ...
                                      + (smallestAngleCorner[1] - line_inter_y)
                                          * (smallestAngleCorner[1]
                                             - line_inter_y)))
                              && (testTriangleAngle (b, &middleAngleCorner[0],
                                                     &middleAngleCorner[1],
                                                     &largestAngleCorner[0],
                                                     &largestAngleCorner[1],
                                                     &petal_slab_inter_x_first,
                                                     &petal_slab_inter_y_first)
                                  != 0)
                              && minDistanceToNeigbor (m, b,
                                                       petal_slab_inter_x_first,
                                                       petal_slab_inter_y_first,
                                                       &neighborotri)
                                   > minDistanceToNeigbor (m, b, line_inter_x,
                                                           line_inter_y,
                                                           &neighborotri))
                            {
                              //
                              /// check the neighbor's vertices also, which one
                              /// if better

                              // slab and petal intersection is advised
                              dxFirstSuggestion
                                = petal_slab_inter_x_first - torg[0];
                              dyFirstSuggestion
                                = petal_slab_inter_y_first - torg[1];
                            }
                          else
                            { // slab intersection point is further away
                              if (testTriangleAngle (b, &middleAngleCorner[0],
                                  ...
                                  != 0)
                                {
                                  ...
                                  // intersection point
                                  d = sqrt (
                                    (line_inter_x - myCircumcenter[0])
                                      * (line_inter_x - myCircumcenter[0])
                                    + (line_inter_y - myCircumcenter[1])
                                        * (line_inter_y - myCircumcenter[1]));
                                  // then find the vector going from
                                  // intersection point to circumcenter
                                  ax = myCircumcenter[0] - line_inter_x;
                                  ay = myCircumcenter[1] - line_inter_y;

                                  ax = ax / d;
                                  ay = ay / d;
                                  // now calculate the new intersection point
                                  // which is perturbated towards the
                                  // circumcenter
                                  line_inter_x = line_inter_x
                                                 + ax * pertConst
                                                     * sqrt (shortestEdgeDist);
                                  line_inter_y = line_inter_y
                                                 + ay * pertConst
                                                     * sqrt (shortestEdgeDist);
                                  if (testTriangleAngle (b,
                                                         &middleAngleCorner[0],
                                                         &middleAngleCorner[1],
                                                         &largestAngleCorner[0],
                                                         &largestAngleCorner[1],
                      ...
                      else
                        {
                          ...
                          if (testTriangleAngle (b, &middleAngleCorner[0],
                                                 ...
                                                 &largestAngleCorner[0],
                                                 &largestAngleCorner[1],
                                                 &neighborCircumcenter[0],
                                                 &neighborCircumcenter[1])
                              != 0)
                            {
                              // go back to circumcenter
                              dxFirstSuggestion = dx;
                    ...
                    { // there is no voronoi vertex between intersection point
                      ...
                      // intersection point
                      pointBetweenPoints (inter_x, inter_y, myCircumcenter[0],
                                          myCircumcenter[1], line_inter_x,
                                          line_inter_y, line_result);
                      if (fabs (line_result[0] - 1.0) <= compConst
                          && line_p[0] > 0.0)
                        {
                          // check if we can go further by picking the slab line
                      ...
                      else
                        {
                          ...
                          if (testTriangleAngle (b, &largestAngleCorner[0],
                              ...
                              != 0)
                            {
                              // printf("testtriangle returned false! bad
                              // triangle\n");
                              // if it is inside feasible region, then insert v2
                              // apply perturbation
                              // find the distance between circumcenter and
                              ...
                              d = sqrt ((inter_x - myCircumcenter[0])
                                        ...
                                        + (inter_y - myCircumcenter[1])
                                            * (inter_y - myCircumcenter[1]));
                              // then find the vector going from intersection
                              // point to circumcenter
                              ax = myCircumcenter[0] - inter_x;
                              ay = myCircumcenter[1] - inter_y;

                              ax = ax / d;
                              ay = ay / d;
                              // now calculate the new intersection point which
                              // is perturbated towards the circumcenter
                              inter_x
                                = inter_x
                                  + ax * pertConst * sqrt (shortestEdgeDist);
                              inter_y
          ...
          if (neighborNotFound_second == 0)
            {
              ...
              //----------------------hale new second direction:for slab
              //calculation---------------//
              // calculate the intersection of angle lines and Voronoi
              ...
              // rotate the vector around largestAngleCorner in ccw by maxangle
              // degrees
                          ...
                          && line_p[0] > 0.0)
                        {
                          ...
                          if (((smallestAngleCorner[0]
                              ...
                              && minDistanceToNeigbor (
                                   m, b, petal_slab_inter_x_second,
                                   petal_slab_inter_y_second, &neighborotri)
                                   > minDistanceToNeigbor (m, b, line_inter_x,
                                                           line_inter_y,
                                                           &neighborotri))
                            {
                              // slab and petal intersection is advised
                              dxSecondSuggestion
                                = petal_slab_inter_x_second - torg[0];
                              dySecondSuggestion
                                = petal_slab_inter_y_second - torg[1];
                            }
                          else
                            { // slab intersection point is further away
                              if (testTriangleAngle (b, &middleAngleCorner[0],
          ...
          if (isObtuse == 1)
            {
              if (neighborNotFound_first == 1 && neighborNotFound_second == 1)
                ...
                {
                  // obtuse: check if the other direction works
                  if (justAcute * ((smallestAngleCorner[0]
                                    - (dxSecondSuggestion + torg[0]))
                                     * (smallestAngleCorner[0]
                                        - (dxSecondSuggestion + torg[0]))
                                   + (smallestAngleCorner[1]
...
// 123: shortest: aodist	// 213: shortest: dadist	// 312: shortest:
// dodist
//	middle: dadist 		//	middle: aodist 		//
//middle: aodist
//	longest: dodist		//	longest: dodist		//
//longest: dadist
// 132: shortest: aodist 	// 231: shortest: dadist 	// 321: shortest:
// dodist
//	middle: dodist 		//	middle: dodist 		//
//middle: dadist
//	longest: dadist		//	longest: aodist		//
//longest: aodist
//
//---------------------------------------------------------------------------------//
int
longestShortestEdge (REAL aodist, REAL dadist, REAL dodist)
{
...
//---------------------------------------------------------------------------------//
int
doSmoothing (mesh *m, behavior *b, struct otri badotri, vertex torg,
             vertex tdest, vertex tapex, REAL *newloc)
{
  ...
  if (numpoints_p != 0 && polygonAngles (b, numpoints_p, points_p) == 0)
    {
      // newLocFound = getPetalIntersection(m, b, numpoints_p, points_p,
      // newloc);
      // newLocFound = getPetalIntersectionBruteForce(m, b,numpoints_p,
      // points_p, newloc,torg[0],torg[1]);
      if (b->maxangle == 0.0)
        {
          newLocFound = getWedgeIntersectionWithoutMaxAngle (m, b, numpoints_p,
                                                             points_p, newloc);
        }
      else
        {
          newLocFound = getWedgeIntersectionWithMaxAngle (m, b, numpoints_p,
                                                          points_p, newloc);
        }
  ...
  if (numpoints_q != 0 && polygonAngles (b, numpoints_q, points_q) == 0)
    {
      ...
      // newLocFound = getPetalIntersectionBruteForce(m, b,numpoints_q,
      // points_q, newloc,tapex[0],tapex[1]);
      if (b->maxangle == 0.0)
        {
          newLocFound = getWedgeIntersectionWithoutMaxAngle (m, b, numpoints_q,
                                                             points_q, newloc);
        }
      else
        {
          newLocFound = getWedgeIntersectionWithMaxAngle (m, b, numpoints_q,
                                                          points_q, newloc);
        }
  ...
  if (numpoints_r != 0 && polygonAngles (b, numpoints_r, points_r) == 0)
    {
      ...
      // newLocFound = getPetalIntersectionBruteForce(m, b,numpoints_r,
      // points_r, newloc,tdest[0],tdest[1]);
      if (b->maxangle == 0.0)
        {
          newLocFound = getWedgeIntersectionWithoutMaxAngle (m, b, numpoints_r,
                                                             points_r, newloc);
        }
      else
        {
          newLocFound = getWedgeIntersectionWithMaxAngle (m, b, numpoints_r,
                                                          points_r, newloc);
        }
  ...
  //////////// AFTER FINISH CHECKING EVERY POSSIBILITY, CHOOSE ANY OF THE
  ///AVAILABLE ONE //////////////////////
  if (num_pos > 0)
    {
      if (flag1 > 0)
...
// returns list of points and the number of points on the star of the given
// point
//---------------------------------------------------------------------------------//
int
getStarPoints (mesh *m, struct otri badotri, vertex p, vertex q, vertex r,
               int whichPoint, REAL *points)
{
  ...
  do
    {
      ...
      if (getNeighborsVertex (m, tempotri, first_x, first_y, second_x, second_y,
                              returnPoint, &neighotri)
          == 0)
        {
          // go to next triangle
          tempotri = neighotri;
...
//---------------------------------------------------------------------------------//
int
getNeighborsVertex (mesh *m, struct otri badotri, REAL first_x, REAL first_y,
                    REAL second_x, REAL second_y, REAL *thirdpoint,
                    struct otri *neighotri)
{
  ...
  for (badotri.orient = 0; badotri.orient < 3; badotri.orient++)
    {
      ...
      /* first check if the neighbor is nonexistent, since it can be on the
       * border */
      if ((neighbor.tri != m->dummytri))
        {
          ...
          if ((neighborvertex_1[0] == neighborvertex_2[0]
               && neighborvertex_1[1] == neighborvertex_2[1])
              || (neighborvertex_2[0] == neighborvertex_3[0]
                  && neighborvertex_2[1] == neighborvertex_3[1])
              || (neighborvertex_1[0] == neighborvertex_3[0]
                  && neighborvertex_1[1] == neighborvertex_3[1]))
            {
      ...
      if (((firstVertexMatched == 11)
           && (secondVertexMatched == 22 || secondVertexMatched == 23))
          || ((firstVertexMatched == 12)
              && (secondVertexMatched == 21 || secondVertexMatched == 23))
          || ((firstVertexMatched == 13)
              && (secondVertexMatched == 21 || secondVertexMatched == 22)))
        break;
...
//---------------------------------------------------------------------------------//
int
getWedgeIntersectionWithoutMaxAngle (mesh *m, behavior *b, int numpoints,
                                     REAL *points, REAL *newloc)
{
  ...
  for (i = 0; i < numpoints * 2; i = i + 2)
    {
      ...
      // petalx[i/2],petaly[i/2], petalr[i/2]);
      ...
      // distance between xmid and petal center
      dist = sqrt ((petalx[i / 2] - xmid) * (petalx[i / 2] - xmid)
      ...
      // cw rotation about (x,y) [ux*cosX + uy*sinX + x - x*cosX - y*sinX;
      // -ux*sinX + uy*cosX + y + x*sinX - y*cosX]
      // ccw rotation matrix [cosX -sinX; sinX cosX]
      // ccw rotation about (x,y) [ux*cosX - uy*sinX + x - x*cosX + y*sinX;
      // ux*sinX + uy*cosX + y - x*sinX - y*cosX]
      ...
      x_1 = x1 * cos (alpha) - y1 * sin (alpha) + x0 - x0 * cos (alpha)
            + y0 * sin (alpha);
      y_1 = x1 * sin (alpha) + y1 * cos (alpha) + y0 - x0 * sin (alpha)
            - y0 * cos (alpha);
      ...
      x_2 = x0 * cos (alpha) + y0 * sin (alpha) + x1 - x1 * cos (alpha)
            - y1 * sin (alpha);
      y_2 = -x0 * sin (alpha) + y0 * cos (alpha) + y1 + x1 * sin (alpha)
            - y1 * cos (alpha);
      // add these to wedges list as lines in order
      wedges[i * 16 + 4] = x_2;
      wedges[i * 16 + 5] = y_2;
      ...
      for (j = 1; j < 4; j++)
        {
          // rotate the vector around (petalx,petaly) in cw by (60 - alpha)*j
          // degrees
          x_3 = x3 * cos ((PI / 3.0 - alpha) * j)
                + y3 * sin ((PI / 3.0 - alpha) * j) + petalx[i / 2]
                - petalx[i / 2] * cos ((PI / 3.0 - alpha) * j)
                - petaly[i / 2] * sin ((PI / 3.0 - alpha) * j);
          y_3 = -x3 * sin ((PI / 3.0 - alpha) * j)
                + y3 * cos ((PI / 3.0 - alpha) * j) + petaly[i / 2]
                + petalx[i / 2] * sin ((PI / 3.0 - alpha) * j)
                - petaly[i / 2] * cos ((PI / 3.0 - alpha) * j);
          // add these to wedges list as lines in order
      ...
      for (j = 1; j < 4; j++)
        {
          // rotate the vector around (petalx,petaly) in ccw by (60 - alpha)*j
          // degrees
          x_4 = x3 * cos ((PI / 3.0 - alpha) * j)
                - y3 * sin ((PI / 3.0 - alpha) * j) + petalx[i / 2]
                - petalx[i / 2] * cos ((PI / 3.0 - alpha) * j)
                + petaly[i / 2] * sin ((PI / 3.0 - alpha) * j);
          y_4 = x3 * sin ((PI / 3.0 - alpha) * j)
                + y3 * cos ((PI / 3.0 - alpha) * j) + petaly[i / 2]
                - petalx[i / 2] * sin ((PI / 3.0 - alpha) * j)
                - petaly[i / 2] * cos ((PI / 3.0 - alpha) * j);
          ...
          if ((p1[0] == 1.0))
            {
              ...
              // printf("INITIAL POLY [%.12f, %.12f;%.12f, %.12f;%.12f,
  ...
  /// HALF PLANE INTERSECTION: START SPLITTING THE INITIAL POLYGON TO FIND
  /// FEASIBLE REGION
  if (numpoints != 0)
    {
      ...
      for (j = 0; j < 32; j = j + 4)
        {
          numpolypoints
            = halfPlaneIntersection (num, initialConvexPoly, wedges[32 * s + j],
                                     wedges[32 * s + 1 + j],
                                     wedges[32 * s + 2 + j],
                                     wedges[32 * s + 3 + j]);
          if (numpolypoints == 0)
      ...
      while (count < numpoints - 1)
        {
          for (j = 0; j < 32; j = j + 4)
            {
              numpolypoints
                = halfPlaneIntersection (num, initialConvexPoly,
                                         wedges[32 * (i + s * flag) + j],
                                         wedges[32 * (i + s * flag) + 1 + j],
                                         wedges[32 * (i + s * flag) + 2 + j],
      ...
      /// IF THERE IS A FEASIBLE INTERSECTION POLYGON, FIND ITS CENTROID AS THE
      /// NEW LOCATION
      ...
      if (b->fixedarea)
        {
          ...
          // 			if(testTriangleAngleArea(m,b,&newloc[0],&newloc[1],
          // &points[j], &points[j+1], &points[j+2], &points[j+3] )){
          // 				numBadTriangle++;
          // 			}
          // 		}
          // 		if(testTriangleAngleArea(m,b, &newloc[0],&newloc[1],
          // &points[0], &points[1], &points[numpoints*2-2],
        ...
        {
          // printf("yes, we found a feasible region num: %d newloc
          // (%.12f,%.12f)\n", numpolypoints, newloc[0], newloc[1]);
          // 	for(i = 0; i < 2*numpolypoints; i = i+2){
          // 		printf("point %d) (%.12f,%.12f)\n", i/2,
          // initialConvexPoly[i], initialConvexPoly[i+1]);
          // 	}
          // 	printf("numpoints %d\n",numpoints);
          return 1;
        }
...
//---------------------------------------------------------------------------------//
int
getWedgeIntersectionWithMaxAngle (mesh *m, behavior *b, int numpoints,
                                  REAL *points, REAL *newloc)
{
  REAL x0, y0, x1, y1, x2, y2;
  // REAL compConst = 0.01; // for comparing real numbers
  ...
  int numpolypoints;
  int
    howManyPoints; // keeps the number of points used for representing the wedge
  REAL line345 = 4.0,
       line789 = 4.0; // flag keeping which line to skip or construct
  ...
  for (i = 0; i < numpoints * 2; i = i + 2)
    {
      ...
      //   	printf("POLYGON POINTS (p,q) #%d (%.12f, %.12f) (%.12f,
      //   %.12f)\n", i/2, x0, y0,x1, y1);
      ...
      /// DETERMINE HOW MANY POINTS TO USE ACCORDING TO THE MINANGLE-MAXANGLE
      /// COMBINATION
      // petal center angle
      ...
      for (j = 1; j < line345; j++)
        {
          if (line345 == 1)
            continue;
          // rotate the vector around (petalx,petaly) in cw by (alpha/3.0)*j
          // degrees
          x_3 = x3 * cos ((alpha / (line345 - 1.0)) * j)
                + y3 * sin (((alpha / (line345 - 1.0)) * j)) + petalx[i / 2]
                - petalx[i / 2] * cos (((alpha / (line345 - 1.0)) * j))
                - petaly[i / 2] * sin (((alpha / (line345 - 1.0)) * j));
          y_3 = -x3 * sin (((alpha / (line345 - 1.0)) * j))
                + y3 * cos (((alpha / (line345 - 1.0)) * j)) + petaly[i / 2]
                + petalx[i / 2] * sin (((alpha / (line345 - 1.0)) * j))
                - petaly[i / 2] * cos (((alpha / (line345 - 1.0)) * j));
          // add these to wedges list as lines in order
      ...
      uy = y0 - y1;
      // rotate the vector around q = (x1,y1) in cw by alpha degrees
      x_5 = x0 * cos (b->maxangle * PI / 180.0)
            + y0 * sin (b->maxangle * PI / 180.0) + x1
            - x1 * cos (b->maxangle * PI / 180.0)
            - y1 * sin (b->maxangle * PI / 180.0);
      y_5 = -x0 * sin (b->maxangle * PI / 180.0)
            + y0 * cos (b->maxangle * PI / 180.0) + y1
            + x1 * sin (b->maxangle * PI / 180.0)
            - y1 * cos (b->maxangle * PI / 180.0);
      wedges[i * 20 + 20] = x1;
      wedges[i * 20 + 21] = y1;
      wedges[i * 20 + 22] = x_5;
      wedges[i * 20 + 23] = y_5;
      ...
      for (j = 1; j < line789; j++)
        {
          if (line789 == 1)
            continue;
          // rotate the vector around (petalx,petaly) in ccw by (alpha/3.0)*j
          // degrees
          x_4 = x3 * cos ((alpha / (line789 - 1.0)) * j)
                - y3 * sin ((alpha / (line789 - 1.0)) * j) + petalx[i / 2]
                - petalx[i / 2] * cos ((alpha / (line789 - 1.0)) * j)
                + petaly[i / 2] * sin ((alpha / (line789 - 1.0)) * j);
          y_4 = x3 * sin ((alpha / (line789 - 1.0)) * j)
                + y3 * cos ((alpha / (line789 - 1.0)) * j) + petaly[i / 2]
                - petalx[i / 2] * sin ((alpha / (line789 - 1.0)) * j)
                - petaly[i / 2] * cos ((alpha / (line789 - 1.0)) * j);
      ...
      uy = y1 - y0;
      // rotate the vector around p = (x0,y0) in ccw by alpha degrees
      x_6 = x1 * cos (b->maxangle * PI / 180.0)
            - y1 * sin (b->maxangle * PI / 180.0) + x0
            - x0 * cos (b->maxangle * PI / 180.0)
            + y0 * sin (b->maxangle * PI / 180.0);
      y_6 = x1 * sin (b->maxangle * PI / 180.0)
            + y1 * cos (b->maxangle * PI / 180.0) + y0
            - x0 * sin (b->maxangle * PI / 180.0)
            - y0 * cos (b->maxangle * PI / 180.0);
      wedges[i * 20 + 36] = x_6;
      wedges[i * 20 + 37] = y_6;
      wedges[i * 20 + 38] = x0;
      wedges[i * 20 + 39] = y0;
      ...
      if (i == 0)
        {
          ...
          // 			printf("real INITIAL POLY [%.12f, %.12f;%.12f,
          // %.12f;%.12f, %.12f;%.12f, %.12f;%.12f, %.12f;%.12f, %.12f;%.12f,
      ...
      for (j = 0; j < 40; j = j + 4)
        {
          ...
          if (howManyPoints == 4 && (j == 8 || j == 12 || j == 16 || j == 24
                                     || j == 28 || j == 32))
            {
              continue;
            }
          else if (howManyPoints == 6
                   && (j == 12 || j == 16 || j == 28 || j == 32))
            {
              continue;
            }
          ...
          // 			printf("%d 1 INITIAL POLY [%.12f, %.12f;%.12f,
          // %.12f;%.12f, %.12f;%.12f, %.12f;%.12f, %.12f;%.12f, %.12f;%.12f,
          // %.12f;%.12f, %.12f;%.12f, %.12f;%.12f, %.12f;]\n",num,
          // initialConvexPoly[0],initialConvexPoly[1],initialConvexPoly[2],initialConvexPoly[3],initialConvexPoly[4],initialConvexPoly[5],initialConvexPoly[6],initialConvexPoly[7],initialConvexPoly[8],initialConvexPoly[9],initialConvexPoly[10],initialConvexPoly[11],initialConvexPoly[12],initialConvexPoly[13],initialConvexPoly[14],initialConvexPoly[15],initialConvexPoly[16],initialConvexPoly[17],initialConvexPoly[18],initialConvexPoly[19]);
          // 			printf("line (%f, %f) (%f,
          // %f)\n",wedges[40*s+j],wedges[40*s+1+j], wedges[40*s+2+j],
          // wedges[40*s+3+j]);
          numpolypoints
            = halfPlaneIntersection (num, initialConvexPoly, wedges[40 * s + j],
                                     wedges[40 * s + 1 + j],
                                     wedges[40 * s + 2 + j],
                                     wedges[40 * s + 3 + j]);
          ...
          for (j = 0; j < 40; j = j + 4)
            {
              ...
              /*printf("%d 2 INITIAL POLY [%.12f, %.12f;%.12f, %.12f;%.12f,
              %.12f;%.12f, %.12f;%.12f, %.12f;%.12f, %.12f;%.12f, %.12f;%.12f,
              %.12f;%.12f, %.12f;%.12f, %.12f;]\n",numpolypoints,
              initialConvexPoly[0],initialConvexPoly[1],initialConvexPoly[2],initialConvexPoly[3],initialConvexPoly[4],initialConvexPoly[5],initialConvexPoly[6],initialConvexPoly[7],initialConvexPoly[8],initialConvexPoly[9],initialConvexPoly[10],initialConvexPoly[11],initialConvexPoly[12],initialConvexPoly[13],initialConvexPoly[14],initialConvexPoly[15],initialConvexPoly[16],initialConvexPoly[17],initialConvexPoly[18],initialConvexPoly[19]);
              printf("line (%.20f, %.20f) (%.20f,
              %.20f)\n",wedges[40*(i+s*flag)+j],wedges[40*(i+s*flag)+1+j],wedges[40*(i+s*flag)+2+j],
              wedges[40*(i+s*flag)+3+j]);*/
              numpolypoints
                = halfPlaneIntersection (num, initialConvexPoly,
                                         wedges[40 * (i + s * flag) + j],
                                         wedges[40 * (i + s * flag) + 1 + j],
                                         wedges[40 * (i + s * flag) + 2 + j],
                                         wedges[40 * (i + s * flag) + 3 + j]);
      ...
      if (b->maxangle != 0.0)
        {
          ...
          for (j = 0; j < numpoints * 2 - 2; j = j + 2)
            {
              if (testTriangleAngle (b, &newloc[0], &newloc[1], &points[j],
          ...
          if (testTriangleAngle (b, &newloc[0], &newloc[1], &points[0],
                                 &points[1], &points[numpoints * 2 - 2],
                                 &points[numpoints * 2 - 1]))
            {
              numBadTriangle++;
            }
              ...
              for (e = 1; e < n; e = e + 1)
                {
                      ...
                      if (i == k)
                        {
                          ...
                          newloc[1]
                            = newloc[1] + 0.1 * e * weight * points[i + 1];
                        }
                      else
                        {
                          weight = (1.0 - 0.1 * e * weight)
                  ...
                  for (j = 0; j < numpoints * 2 - 2; j = j + 2)
                    {
                      if (testTriangleAngle (b, &newloc[0], &newloc[1],
                                             &points[j], &points[j + 1],
...
//---------------------------------------------------------------------------------//
int
polygonAngles (behavior *b, int numpoints, REAL *points)
{
  int i;
  for (i = 0; i < numpoints; i++)
    {
      if (i == numpoints - 1)
        {
          if (testPolygonAngle (b, &points[i * 2], &points[i * 2 + 1],
...
// Returns 1, if it is a BAD polygon corner, returns 0 if it is a GOOD polygon
// corner
//---------------------------------------------------------------------------------//
int
testPolygonAngle (behavior *b, REAL *x1, REAL *y1, REAL *x2, REAL *y2, REAL *x3,
                  REAL *y3)
{
  ...
  // printf("angle: %f 2*minangle = %f\n",acos(cosAngle)*180/PI,
  // 2*acos(sqrt(b->goodangle))*180/PI);
...
//---------------------------------------------------------------------------------//
void
lineLineIntersection (REAL x1, REAL y1, REAL x2, REAL y2, REAL x3, REAL y3,
                      REAL x4, REAL y4, REAL *p)
{
  ...
  if (fabs (denom - 0.0) < compConst
      && (fabs (u_b - 0.0) < compConst && fabs (u_a - 0.0) < compConst))
    {
      p[0] = 0.0;
    }
...
//---------------------------------------------------------------------------------//
int
halfPlaneIntersection (int numvertices, REAL *convexPoly, REAL x1, REAL y1,
                       REAL x2, REAL y2)
{
...
//---------------------------------------------------------------------------------//
int
splitConvexPolygon (int numvertices, REAL *convexPoly, REAL x1, REAL y1,
                    REAL x2, REAL y2, REAL *polys[])
{
  /*
  ...
  */
  ...
  // for debugging
  int case1 = 0, case2 = 0, case3 = 0, case31 = 0, case32 = 0, case33 = 0,
      case311 = 0, case3111 = 0;
  // intersect all edges of poly with line
  for (i = 0; i < 2 * numvertices; i = i + 2)
    {
      int j = (i + 2 >= 2 * numvertices) ? 0 : i + 2;
      lineLineSegmentIntersection (x1, y1, x2, y2, convexPoly[i],
                                   convexPoly[i + 1], convexPoly[j],
                                   convexPoly[j + 1], p);
      ...
      // ... or if the intersection is a point
      else
        {
          ...
          if (fabs (p[1] - convexPoly[j]) <= compConst
              && fabs (p[2] - convexPoly[j + 1]) <= compConst)
            {
              // debug
              case31++;
              if (state == 1)
              ...
              else if (state == 0)
                {
                  ...
                  if (i + 4 < 2 * numvertices)
                    {
                      int s1 = linePointLocation (x1, y1, x2, y2, convexPoly[i],
                                                  convexPoly[i + 1]);
                      int s2
                        = linePointLocation (x1, y1, x2, y2, convexPoly[i + 4],
                                             convexPoly[i + 5]);
          ...
          else if (!(fabs (p[1] - convexPoly[i]) <= compConst
                     && fabs (p[2] - convexPoly[i + 1]) <= compConst))
            {
              // debug
              case32++;
              poly1counter++;
  ...
  if (state != 0 && state != 2)
    {
      ...
      // 		printf("case1: %d\ncase2: %d\ncase3: %d\ncase31: %d
      // case311: %d case3111: %d\ncase32: %d\ncase33: %d\n", case1, case2,
      // case3, case31, case311, case3111, case32, case33);
...
//---------------------------------------------------------------------------------//
int
linePointLocation (REAL x1, REAL y1, REAL x2, REAL y2, REAL x, REAL y)
{
  REAL z;
...
// Given four points representing one line and a line segment, returns the
// intersection point
// referenced to: http://local.wasp.uwa.edu.au/~pbourke/geometry/
//---------------------------------------------------------------------------------//
void
lineLineSegmentIntersection (REAL x1, REAL y1, REAL x2, REAL y2, REAL x3,
                             REAL y3, REAL x4, REAL y4, REAL *p)
{
  ...
  // if(fabs(denom-0.0) < compConst && (fabs(u_b-0.0) < compConst &&
  // fabs(u_a-0.0) < compConst)){
  // printf("denom %.20f u_b  %.20f u_a  %.20f\n",denom, u_b, u_a);
  if (fabs (denom - 0.0) < compConst)
    {
      if (fabs (u_b - 0.0) < compConst && fabs (u_a - 0.0) < compConst)
        {
          p[0] = 2.0; // if denominator and numerator equal to zero, lines are
                      // coincident
        }
      else
        {
  ...
  else
    {
      ...
      if (u_b < -compConst || u_b > 1.0 + compConst)
        { // check if it is on the line segment
          // 		printf("line (%.20f, %.20f) (%.20f, %.20f) line seg (%.20f,
...
//---------------------------------------------------------------------------------//
void
findPolyCentroid (int numpoints, REAL *points, REAL *centroid)
{
  int i;
  centroid[0] = 0.0;
  centroid[1] = 0.0;
...
void
circleLineIntersection (REAL x1, REAL y1, REAL x2, REAL y2, REAL x3, REAL y3,
                        REAL r, REAL *p)
{
  // x1,y1  P1 coordinates [point of line]
  // x2,y2  P2 coordinates [point of line]
...
//---------------------------------------------------------------------------------//
int
chooseCorrectPoint (REAL x1, REAL y1, REAL x2, REAL y2, REAL x3, REAL y3,
                    int isObtuse)
{
...
void
pointBetweenPoints (REAL x1, REAL y1, REAL x2, REAL y2, REAL x, REAL y, REAL *p)
{

  // x1,y1  P1 coordinates [point of line] (point on Voronoi edge -
  // intersection)
  // x2,y2  P2 coordinates [point of line] (circumcenter)
  // x,y    P3 coordinates [point to be compared]	(neighbor's
  // circumcenter)
  //
  // This function returns a pointer array which first index indicates
  // the whether the point is in between the other points, followed by
  // coordinate pairs.

  // now check whether the point is close to circumcenter than intersection
  // point
  // BETWEEN THE POINTS
...
// tests a triangle to see if it satisfies the minimum and/or maximum angle
// condition
// Returns 1, if it is a BAD triangle, returns 0 if it is a GOOD triangle
//---------------------------------------------------------------------------------//
int
testTriangleAngle (behavior *b, REAL *x1, REAL *y1, REAL *x2, REAL *y2,
                   REAL *x3, REAL *y3)
{
  // variables keeping the distance values for the edges
  ...
  if ((apexlen > orglen) && (apexlen > destlen))
    {
      ...
      maxangle
        = (orglen + destlen - apexlen) / (2 * sqrt (orglen) * sqrt (destlen));
    }
  else if (orglen > destlen)
    {
      ...
      maxangle
        = (apexlen + destlen - orglen) / (2 * sqrt (apexlen) * sqrt (destlen));
    }
  else
    {
      ...
      maxangle
        = (apexlen + orglen - destlen) / (2 * sqrt (apexlen) * sqrt (orglen));
    }

  /* Check whether the angle is smaller than permitted. */
  if ((angle > b->goodangle)
      || (b->maxangle != 0.00 && maxangle < b->maxgoodangle))
    {
      return 1; // it is a bad triangle
    }
  return 0; // it is a good triangle
...
// returns the minimum distance to the vertices of the triangle where the given
// vertex located
//---------------------------------------------------------------------------------//
REAL
minDistanceToNeigbor (mesh *m, behavior *b, REAL newlocX, REAL newlocY,
                      struct otri *searchtri)
{
  ...
  if (intersect == ONVERTEX || intersect == OUTSIDE)
    ...
    { // intersect == ONEDGE || intersect == INTRIANGLE
      ...
      dest (horiz, v2);
      apex (horiz, v3);
      d1 = (v1[0] - newvertex[0]) * (v1[0] - newvertex[0])
           + (v1[1] - newvertex[1]) * (v1[1] - newvertex[1]);
      d2 = (v2[0] - newvertex[0]) * (v2[0] - newvertex[0])
           + (v2[1] - newvertex[1]) * (v2[1] - newvertex[1]);
      d3 = (v3[0] - newvertex[0]) * (v3[0] - newvertex[0])
           + (v3[1] - newvertex[1]) * (v3[1] - newvertex[1]);
      vertexdealloc (m, newvertex);
