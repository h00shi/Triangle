...
void
eps_write_header (FILE *file, const char *title, rect *ps)
{
  ...
  fprintf (file, "%%%%BoundingBox:  %d  %d  %d  %d\n", (int) ps->xmin,
           (int) ps->ymin, (int) ps->xmax, (int) ps->ymax);
...
void
eps_draw_clip (FILE *file, rect *ps, rect *clip)
{
  fprintf (file, "save\n");
  fprintf (file, "%%\n");
...
void
eps_draw_edges (FILE *file, mesh *m, rect *ps, rect *ms)
{
          ...
          if ((e.tri < trisym.tri) || (trisym.tri == m->dummytri))
            {
              ...
              fprintf (file, "%d %d %d %d L\n", TX (p[0], ps, ms),
                       TY (p[1], ps, ms), TX (q[0], ps, ms), TY (q[1], ps, ms));
            }
        }
      e.tri = triangletraverse (m);
    }
...
void
eps_draw_segments (FILE *file, mesh *m, rect *ps, rect *ms)
{
  struct osub e;
  vertex p, q;
  ...
  while (e.ss != (subseg *) NULL)
    {
      ...
      fprintf (file, "%d %d %d %d L\n", TX (p[0], ps, ms), TY (p[1], ps, ms),
               TX (q[0], ps, ms), TY (q[1], ps, ms));
...
void
eps_draw_points (FILE *file, mesh *m, rect *ps, rect *ms)
{
  vertex e;
...
void
eps_set_color (FILE *file, float r, float g, float b)
{
...
void
eps_set_stroke (FILE *file, float r, float g, float b, float width)
{
  fprintf (file, "%.3g %.3g %.3g setrgbcolor\n", r, g, b);
...
void
eps_update_metrics (rect *ps, rect *clip, rect *ms)
{
  ...
  if (x_scale < y_scale)
    {
      int delta
        = rint ((ps->xmax - ps->xmin) * (y_scale - x_scale) / (2.0 * y_scale));
    ...
    {
      int delta
        = rint ((ps->ymax - ps->ymin) * (x_scale - y_scale) / (2.0 * x_scale));
