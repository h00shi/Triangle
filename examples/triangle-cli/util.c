...
info ()
{
  ...
  printf ("\n");
  printf (
    "Detailed instructions can be found in the README file or online at\n");
...
void
parsecommandline_legacy (int argc, char **argv, behavior_legacy *b)
{
#define STARTINDEX 1
  int increment;
  int meshnumber;
...
void
check_behavior (behavior *b, behavior_legacy *lb)
{
...
void
print_statistics (context *ctx, int verbose)
{
  int i;
  REAL ratiotable[16];
  quality q;
      ...
      for (i = 0; i < 9; i++)
        {
          printf (
            ...
            i * 10, i * 10 + 10, q.angletable[i], i * 10 + 90, i * 10 + 100,
            q.angletable[i + 9]);
        }
      printf ("\n");
...
void
reset_triangleio (triangleio *io)
{
...
void
check (int code, const char *site)
{
  if (code < 0)
    {
...
void
finishfile (FILE *file, int argc, char **argv)
{
  int i;
...
int
write_nodes (context *ctx, char *filename, int argc, char **argv)
{
  FILE *file;
  int status = 0;
...
int
write_elements (context *ctx, char *filename, int argc, char **argv)
{
  FILE *file;
  int status = 0;
...
int
write_poly (context *ctx, char *filename, triangleio *in, int argc, char **argv)
{
  FILE *file;
  int status = 0;
...
int
write_edges (context *ctx, char *filename, int argc, char **argv)
{
  FILE *file;
  int status = 0;
...
int
write_neighbors (context *ctx, char *filename, int argc, char **argv)
{
  FILE *file;
  int status = 0;
...
int
write_postscript (context *ctx, char *filename, int argc, char **argv)
{
  FILE *file;
  int status = 0;
