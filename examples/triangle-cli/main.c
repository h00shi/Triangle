...
main (int argc, char **argv)
{
  ...
  if (b->poly)
    {
      check (triangle_read_poly (lb.inpolyfilename, &in, &firstnode),
             "read_poly");
    }
  ...
  if (in.numberofpoints == 0)
    {
      ...
      check (triangle_read_nodes (lb.innodefilename, &in, &firstnode),
             "read_nodes");
    }

#ifdef CDT_ONLY
  check (triangle_mesh_create (ctx, &in),
         "mesh_create"); /* Triangulate the vertices. */
#else                    /* not CDT_ONLY */
  if (b->refine)
    {
      /* Read the mesh. */
  ...
  if (lb.nonodewritten || (lb.noiterationnum && !b->poly))
    {
      ...
      // numbernodes(&m, &b);         /* We must remember to number the
      // vertices. */
    }
  else
    {
